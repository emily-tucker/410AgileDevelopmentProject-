...
/**
 * *************************************************************************
 * CLASS Tokenizer ----------------------------------- Description Brakes things
 * into words and pairs them with their type. Brakes a String into words and
 * finds their type to create tokens. The String is the entire question from the
 * user.
 * **************************************************************************
 */
package Token;
...
//        }
    ...
    public static boolean isPreposition(String s) {
        String preopositions = "above before except by from in near of since for between upon with to at after on across into";
    ...
    /**
     * *************************************************************************
     * lexer(String question) ----------------------------------- Description
     *     
* **************************************************************************
     */
    ...
    public static TokenStream tagger(TokenStream toks) {
        ...
        if (toks.peek().type == TokenType.unknown) {
            ...
            if (toks.away(1).type == TokenType.punctuation && toks.away(1).body.equalsIgnoreCase(",")) {//Appostive phrase
                boolean searchNextComma = true;
                int i = 2;
                while (searchNextComma) {
                    if (toks.away(i).type == TokenType.punctuation && toks.away(i).body.equalsIgnoreCase(",") && toks.away(i + 1).type == TokenType.verb) {
                        searchNextComma = false;
                        toks.away(i).type = TokenType.verb;
                        toks.away(i).body = "is";
                    }
                    i++;
                }
            ...
            //beginning of sentence test
            if (toks.away(-1).type == TokenType.EOS && toks.away(1).type == TokenType.verb) {
                toks.peek().type = TokenType.noun;
            }
            if (toks.away(-1).type == TokenType.EOS && toks.away(1).type == TokenType.punctuation && toks.away(2).type == TokenType.noun) {
                toks.peek().type = TokenType.adverb;
            }
        }
        if (toks.peek().type == TokenType.preposition) {
            if (toks.away(1).type == TokenType.unknown || toks.away(-1).type == TokenType.verb) {
                toks.peek().type = TokenType.noun;
            }
            if (toks.away(-1).type == TokenType.noun || toks.away(-1).type == TokenType.pronoun || toks.away(-1).type == TokenType.propernoun) {
                toks.peek().type = TokenType.verb;
            }
            if (toks.away(2).type == TokenType.unknown) {
                if (toks.away(4).type == TokenType.unknown) {
...
//            }
        if (toks.peek().type == TokenType.conjunction) {
            if (toks.away(-2).type == TokenType.verb && toks.away(-1).type == TokenType.punctuation) {
                toks.away(1).type = TokenType.verb;
            }
            if (toks.away(-2).type == TokenType.noun && toks.away(-1).type == TokenType.punctuation) {
